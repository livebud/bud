...
func tryFunction(fn *parser.Function, importPath, dataType string) (*function, error) {
	...
	for _, param := range fn.Params() {
		...
		if err != nil {
			return nil, fmt.Errorf("di: unable to find definition for param %q.%s in %q.%s .\n\t%w", imPath, parser.Unqualify(pt).String(), importPath, dataType, err)
		}
	...
	for _, result := range results {
		...
		if err != nil {
			return nil, fmt.Errorf("di: unable to find definition for result %q.%s in %q.%s .\n\t%w", imPath, parser.Unqualify(rt).String(), importPath, dataType, err)
		}
