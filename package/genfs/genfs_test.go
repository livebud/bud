...
type DiscardCacheGraph struct {
}

var _ genfs.CacheGraph = (*DiscardCacheGraph)(nil)

func (c *DiscardCacheGraph) Get(path string) (entry virtual.Entry, ok bool) {
	return
}

func (c *DiscardCacheGraph) Set(path string, entry virtual.Entry) {
}

func (c *DiscardCacheGraph) Check(from string, checker func(path string) (changed bool)) {
}

func (c *DiscardCacheGraph) Link(from, to string) {
}
...
func TestGenerateFile(t *testing.T) {
	...
	fsys := virtual.Map{}
	log := testlog.New()
	cg := &DiscardCacheGraph{}
	gfs := genfs.New(cg, fsys, log)
...
func TestEmbedOpen(t *testing.T) {
	...
	cg := &DiscardCacheGraph{}
	bfs := genfs.New(cg, fsys, log)
	bfs.FileGenerator("bud/view/index.svelte", &genfs.EmbedFile{
		Data: []byte(`<h1>index</h1>`),
	})
	bfs.FileGenerator("bud/view/about/about.svelte", &genfs.EmbedFile{
		Data: []byte(`<h1>about</h1>`),
	})
	bfs.FileGenerator("bud/public/favicon.ico", &genfs.EmbedFile{
...
func TestGoModGoModEmbed(t *testing.T) {
	...
	cg := &DiscardCacheGraph{}
	bfs := genfs.New(cg, fsys, log)
	bfs.FileGenerator("go.mod", &genfs.EmbedFile{
...
// 	linker := &Linker{}
// 	bfs := genfs.New(cg, fsys, log)
// 	bfs.GenerateFile("bud/command/generate/main.go", func(fsys genfs.FS, file *genfs.File) error {
// 		file.Data = []byte(`
// 			package main
// 			func main() {}
// 		`)
// 		// Services are allowed to be within a directory
// 		bfs.ServeFile("bud/service/transform", func(fsys genfs.FS, file *genfs.File) error {
// 			file.Data = []byte(`transforming: ` + file.Relative())
// 			return nil
// 		})
// 		return nil
// 	})
// 	bfs.GenerateDir("bud/service", func(fsys genfs.FS, dir *genfs.Dir) error {
...
// 	linker := &Linker{}
// 	mountfs := genfs.New(cg, fsys, log)
// 	mountfs.ServeFile("bud/service/transform", func(fsys genfs.FS, file *genfs.File) error {
// 		file.Data = []byte(`transforming: ` + file.Relative())
// 		return nil
// 	})
// 	cache := vcache.Discard
...
// 	dir.FileGenerator("a.txt", &genfs.EmbedFile{Data: []byte("a")})
// 	dir.FileGenerator("b/b.txt", &genfs.EmbedFile{Data: []byte("b")})
// 	code, err := fs.ReadFile(bfs, "a.txt")
// 	is.NoErr(err)
// 	is.Equal(string(code), "a")
