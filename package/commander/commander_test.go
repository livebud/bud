...
func TestHiddenCommand(t *testing.T) {
	...
	actual := new(bytes.Buffer)
	cli := commander.New("cli").Writer(actual)
	cli.Command("one", "one command").Hidden()
	cli.Command("two", "two command")
	ctx := context.Background()
	err := cli.Parse(ctx, []string{"--help"})
	is.NoErr(err)
	is.In(actual.String(), "cli")
	is.In(actual.String(), "[command]")
	is.In(actual.String(), "two")
	is.In(actual.String(), "two command")
	is.NotIn(actual.String(), "one")
	is.NotIn(actual.String(), "one command")
}
