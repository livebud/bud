...
func TestColonSub(t *testing.T) {
	...
	{
		new := cli.Command("new", "new project")
		new.Run(func(ctx context.Context) error {
			trace = append(trace, "new")
			return nil
		})

		{
			newController := new.Command("controller", "new controller")
			newController.Run(func(ctx context.Context) error {
				trace = append(trace, "new:controller")
				return nil
			})
		}
	}
	ctx := context.Background()
	err := cli.Parse(ctx, []string{"new:controller"})
	is.NoErr(err)
	is.Equal(len(trace), 1)
	is.Equal(trace[0], "new:controller")
	isEqual(t, actual.String(), ``)
}
