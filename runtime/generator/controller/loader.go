...
func (l *loader) loadView(controllerKey, actionKey, actionRoute string) *View {
	...
	if err != nil {
		...
		l.Bail(fmt.Errorf("controller: unable read view directory %q .\n\t%w", viewDir, err))
	}
	for _, de := range des {
		name := de.Name()
		ext := path.Ext(name)
...
func (l *loader) loadActionParam(param *parser.Param, nth, numParams int) *ActionParam {
	...
	if err != nil {
		l.Bail(fmt.Errorf("controller: unable to find param definition for %s .\n\t%w", param.Type(), err))
	}
	ap := new(ActionParam)
...
func (l *loader) loadActionResult(order int, result *parser.Result) *ActionResult {
	...
	if err != nil {
		l.Bail(fmt.Errorf("controller: unable to load result definition for %s .\n\t%w", result.Type(), err))
	}
...
func (l *loader) loadActionResultFields(result *parser.Result, def parser.Declaration) (fields []*ActionResultField) {
	...
	for _, field := range stct.PublicFields() {
		...
		if err != nil {
			l.Bail(fmt.Errorf("controller: unable to load definition for field %s in %s .\n\t%w", field.Name(), result.Name(), err))
		}
