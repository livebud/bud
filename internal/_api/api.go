...
func Load(appFS vfs.ReadWritable, options ...Option) (*API, error) {
	option := &option{
		Embed:  false,
		Hot:    true,
		Minify: false,
	...
	genFS := gen.New(appFS)
	// TODO: support gitignore and single-flight caching, need to be able to reset
	// the cache each api.Generate().
	modFinder := mod.New(mod.WithFS(genFS), mod.WithCache(option.Cache))
