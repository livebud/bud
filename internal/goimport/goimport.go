// Package goimport imports Go files from a virtual filesystem. It takes into
// account all the rules around importing source files. This includes:
// - Skipping over *_test.go files
// - Skipping build tags that don't match
//
// TODO: this package should also be able to handle import paths. We have some
// convoluted resolution logic in the parser's selector type definition lookup
// that should probably be handled in this package instead.
package goimport
...
func New(fsys fs.FS) *Importer {
	return &Importer{buildContext(fsys)}
}
...
// Import from a directory in a virtual filesystem
func (i *Importer) Import(dir string) (*build.Package, error) {
	// TODO: figure out how to set the import path correctly to have better error
	// messages
	imported, err := i.context.Import(".", dir, build.ImportMode(0))
	if err != nil {
