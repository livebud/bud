...
func ImportMap(log logs.Log, imports map[string]string) esbuild.Plugin {
	return importMap(log, imports, false)
}

// ExternalImportMap rewrites imports to a different path and marks them as external
func ExternalImportMap(log logs.Log, imports map[string]string) esbuild.Plugin {
	return importMap(log, imports, true)
}

func importMap(log logs.Log, imports map[string]string, external bool) esbuild.Plugin {
