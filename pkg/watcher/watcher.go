...
func (p *eventSet) Flush() *Events {
	...
	for _, event := range p.events {
		switch event.Op {
		case OpCreate:
			events.Created = append(events.Created, event.Path)
		case OpUpdate:
			events.Updated = append(events.Updated, event.Path)
		case OpDelete:
			events.Deleted = append(events.Deleted, event.Path)
		}
	}
	// Sort the paths
	sort.Strings(events.Created)
	sort.Strings(events.Updated)
	sort.Strings(events.Deleted)
	// Clear the events
...
func Watch(ctx context.Context, dir string, fn func(events *Events) error) error {
