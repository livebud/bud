...
func Sync(log logs.Log, from fs.FS, to FS, subpaths ...string) error {
...
func diff(log logs.Log, from fs.FS, to FS, dir string) (ops []syncOp, err error) {
...
func createOps(log logs.Log, from fs.FS, dir string, des []fs.DirEntry) (ops []syncOp, err error) {
	for _, de := range des {
		if de.Name() == "." {
			continue
		}
...
func deleteOps(log logs.Log, dir string, des []fs.DirEntry) (ops []syncOp, err error) {
	for _, de := range des {
...
func updateOps(log logs.Log, from fs.FS, to FS, dir string, des []fs.DirEntry) (ops []syncOp, err error) {
	for _, de := range des {
		if de.Name() == "." {
			continue
		}
...
func apply(log logs.Log, to FS, ops []syncOp) error {
	for _, op := range ops {
		switch op.Type {
		case createType:
