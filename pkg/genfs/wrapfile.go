...
func wrapFile(file fs.File, genfs fs.FS, path string) *wrappedFile {
	return &wrappedFile{
		...
		offset:      0,
		readOnce:    new(once.Once[[]byte]),
		readDirOnce: new(once.Once[[]fs.DirEntry]),
		statOnce:    new(once.Once[fs.FileInfo]),
	}
}
...
type wrappedFile struct {
	...
	// stateful
	offset      int64
	readOnce    *once.Once[[]byte]
	readDirOnce *once.Once[[]fs.DirEntry]
	statOnce    *once.Once[fs.FileInfo]
}
